class VendingMachine:
    ...
    def fillProducts(self):
        ...
        self.__num1 = self.__max1 # self.__num1 = self.__max1
    ...
    def fillCoins(self, c1: int, c2: int):
        if self.__mode == VendingMachine.Mode.OPERATION:
            return VendingMachine.Response.ILLEGAL_OPERATION
        if c1 <= 0 or c1 > self.__maxc1: # if c1 <= 0 or c2 > self.__maxc2:
            return VendingMachine.Response.INVALID_PARAM
        if c2 <= 0 or c2 > self.__maxc2: # if c1 <= 0 or c2 > self.__maxc2:
            return VendingMachine.Response.INVALID_PARAM
        self.__coins1 = c1
        self.__coins2 = c2
        return VendingMachine.Response.OK
    ...
    def enterAdminMode(self, code: int):
        if self.__balance != 0:
            return VendingMachine.Response.CANNOT_PERFORM # return VendingMachine.Response.UNSUITABLE_CHANGE
        if code != self.__id:
            return VendingMachine.Response.INVALID_PARAM
    ...
    def putCoin1(self):
        if self.__mode == VendingMachine.Mode.ADMINISTERING:
            return VendingMachine.Response.ILLEGAL_OPERATION
        if self.__coins1 == self.__maxc1: # if self.__coins2 == self.__maxc2:
            return VendingMachine.Response.CANNOT_PERFORM
        self.__balance += self.__coinval1 # self.__balance += self.__coinval2
        self.__coins1 += 1 # self.__coins2 += 1
        return VendingMachine.Response.OK

    def putCoin2(self):
        if self.__mode == VendingMachine.Mode.ADMINISTERING:
            return VendingMachine.Response.ILLEGAL_OPERATION
        if self.__coins2 == self.__maxc2: # if self.__coins1 == self.__maxc1
            return VendingMachine.Response.CANNOT_PERFORM
        self.__balance += self.__coinval2 # self.__balance += self.__coinval1
        self.__coins2 += 1 # self.__coins1 += 1
        return VendingMachine.Response.OK
